...
public class Susanoo : NinjaModCard
{
    ...
    protected override async Task OnPlay(PlayerChoiceContext choiceContext, CardPlay cardPlay)
    {
        ...
        // 打出后进入隐身。
        await PowerCmd.Apply<StealthPower>(choiceContext, Owner.Creature, 1, Owner.Creature, this);
    }

    protected override void OnUpgrade() => DynamicVars.Damage.UpgradeValueBy(2m); // 7 -> 9

    public override List<(string, string)>? Localization => Lang.Zh
        ? new CardLoc("须佐能乎", $"造成 {DynamicVars.Damage.BaseValue} 点伤害，共 {Hits} 段，每段追加 1 层流血。然后进入隐身状态。")
        : new CardLoc("Susanoo", $"Deal {DynamicVars.Damage.BaseValue} damage {Hits} times; each hit applies 1 Bleed. Then become Stealthed.");
}
